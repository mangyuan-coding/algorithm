                                     ...
                                     char **words, struct WordTreeNode **idx_nodes)
{
    ...
    if (child_idxes != NULL)
    {
        nodes = malloc(sizeof(struct WordTreeNode *) * HASH_COUNT(child_idxes));
...
void clean_nodes(struct WordTreeNode ***deep_nodes, int *deep_node_size, struct IntSet *path_idxes, int deep)
{
    if (deep == 0)
    {
        return;
    ...
    {
        if (deep_nodes[deep - 1][i]->children_size == 0)
        {
            continue;
        }
        int left = 0, right = 0;
        while (right < deep_nodes[deep - 1][i]->children_size)
        {
            if (contains(path_idxes, deep_nodes[deep - 1][i]->children[right]->word_idx))
            {
                if (right > left)
                {
                    deep_nodes[deep - 1][i]->children[left++] = deep_nodes[deep - 1][i]->children[right++];
                }
                else
                {
                    left++;
        ...
        deep_nodes[deep - 1][i]->children_size = left;
    }
    path_idxes = remove_all(path_idxes);
    for (int i = 0; i < deep_node_size[deep - 1]; i++)
    {
        if (deep_nodes[deep - 1][i]->children_size > 0)
        {
            path_idxes = add_int(path_idxes, deep_nodes[deep - 1][i]->word_idx);
        }
    }
    clean_nodes(deep_nodes, deep_node_size, path_idxes, deep - 1);
}
                    ...
                    int *returnSize, int **returnColumnSizes)
{
    ...
    int *deep_node_size = malloc(sizeof(int) * wordListSize);

    bool *has_find_end = calloc(1, sizeof(bool));
    while ((*node_size) > 0)
    {
        deep_nodes[deep++] = nodes;
        deep_node_size[deep - 1] = (*node_size);
    ...
    struct IntSet *path_idxes = NULL;
    path_idxes = add_int(path_idxes, end_word_idx);
    clean_nodes(deep_nodes, deep_node_size, path_idxes, deep);
